...
def constructBayesNet(gameState):
    ...
    """ NOT SURE IF I NEED THIS SO LETS HOLD OFF 
    UPDATS: YES NEEEDED
    """
    for obs in obsVars:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Get new distributtion
    final_distribution = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    max_prob = 0
    max_loc = None
    # get most likely prob
    for location in final_distribution.getAllPossibleAssignmentDicts():
        if final_distribution.getProbability(location) > max_prob:
            max_prob = final_distribution.getProbability(location)
            max_loc = location
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        """
        "*** YOUR CODE HERE ***"
        # Get the new marginalized distirbution
        distribution = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR,
                                                                                GHOST_HOUSE_VAR], evidence,
                                                                eliminationOrder)

        # Set probabilities for special cases above to 0 so pip8 leaves me alone
        prob_l_good = 0
        prob_r_good = 0

        # Go through the dictionaries for the two cases we want to consider and get the probs
        for row in distribution.getAllPossibleAssignmentDicts():
            if (FOOD_HOUSE_VAR, TOP_RIGHT_VAL) in row.items() and (GHOST_HOUSE_VAR, TOP_LEFT_VAL) in row.items():
                prob_r_good = distribution.getProbability(row)
            if (FOOD_HOUSE_VAR, TOP_LEFT_VAL) in row.items() and (GHOST_HOUSE_VAR, TOP_RIGHT_VAL) in row.items():
                prob_l_good = distribution.getProbability(row)

        # set expectation: reward * prob + penalty * prob
        leftExpectedValue = (prob_l_good * WON_GAME_REWARD) + (prob_r_good * GHOST_COLLISION_REWARD)
        rightExpectedValue = (prob_r_good * WON_GAME_REWARD) + (prob_l_good * GHOST_COLLISION_REWARD)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        new_evidence_pairs = self.getExplorationProbsAndOutcomes(evidence)
        for prob, new_evidence in new_evidence_pairs:
            new_vals = self.computeEnterValues(new_evidence, enterEliminationOrder)
            best_val = max(new_vals)
            expectedValue += prob * best_val
        "*** END YOUR CODE HERE ***"
